...
Despite the increase in set complexity, as indicated by the number of unique parts, there is a noted decline in the average number of minifigures included per set in recent years, suggesting a shift in focus or a response to changing consumer preferences.
...
minifigs_per_set_year <- minifigs_data %>%
 group_by(year, set_num) %>%
 summarize(total_minifigs = n_distinct(fig_num), .groups = 'drop') %>%
 ungroup()

minifigs_trend <- minifigs_per_set_year %>%
  group_by(year) %>%
  summarize(average_minifigs = mean(total_minifigs, na.rm = TRUE), .groups = 'drop')

fig <- plot_ly(minifigs_trend, x = ~year, y = ~average_minifigs, type = 'scatter', mode = 'lines+markers',
               hoverinfo = 'y+text', text = ~paste("Year:", year, "<br>Avg. Minifigs:", average_minifigs))


fig <- fig %>% layout(title = 'Average Number of Minifigures per Set Over Time',
                      xaxis = list(title = 'Year'),
                      yaxis = list(title = 'Average Number of Minifigures'))
fig
